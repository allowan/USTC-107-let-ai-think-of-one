...
| 位置 | 职责 |
|---|---|
| `pages/ChatPage.tsx` | SSE 流式对话（fetch + ReadableStream 解析）、Markdown 渲染、首轮对话自动摘要生成话题标题 |
| `pages/PersonalDataPage.tsx` | 个人知识库管理（按来源聚合、增删改） |
...
前端解析约定：逐行 `JSON.parse` 单独容错（坏行跳过不杀流）；流结束时 flush 解码器残余；切换话题或组件卸载时主动 `abort`。
