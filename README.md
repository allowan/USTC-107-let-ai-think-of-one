...
- **时间感知** — 从通知中确定性抽取截止日/发布日/类别存入时间索引（`events.db`），“最近有什么要截止的报名”按真实日期排序回答，不遗漏措辞不同但时间临近的通知
...
用户提问
  → Agent（main.py）决定调用哪些工具
     ...
     ├── get_upcoming_events                          → 按截止日查询即将到期的校园事件（events.db 时间索引）
...
- [x] **日程解析工具（第一阶段）** — 从通知中确定性抽取截止日/发布日/类别/适用对象，`get_upcoming_events` 按真实日期查询“未来 N 天即将截止的事件”（离线、不依赖 LLM）；自动生成日历事件、地点抽取为后续阶段
