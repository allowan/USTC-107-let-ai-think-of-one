...
| 生命周期 | `server/lifespan.py` | 启动时初始化 ChatService，关闭时收尾 agent 连接 |
...
| 服务 | 职责 |
|---|---|
| `chat_service.py` | Agent 生命周期（默认/按用户缓存）、SSE 事件流、checkpoint 删除与损坏重试 |
...
| Method | Path | 说明 |
|---|---|---|
| GET | `/api/health` | 健康检查（agent / LLM / ChromaDB，全部在线程池探测不阻塞事件循环） |
| GET | `/api/topics` | 话题列表 |
